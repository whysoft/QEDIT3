/*******************************************************************************
 * Project:  neb
 2020-11-03	Get2 如果值是null则空串， true/false小写。
 2020-12-26 加一个重载的 Get2 
 2021-02-20 增加Add2
 ******************************************************************************/
...
bool CJsonObject::Get2(const std::string& strKey, std::string& strValue) const
{
    cJSON* pJsonStruct = NULL;
    if (m_pJsonData != NULL)
    {
    ...
    if (pJsonStruct->type != cJSON_String)
    {
		...
		if (pJsonStruct->type == cJSON_Int)		// 对于整数，默认采用long
		{
			std::stringstream stream;
			stream << (long)(pJsonStruct->valueint);
			strValue = stream.str();
			return(true);
		}
		...
		if (pJsonStruct->type == cJSON_False)
		{
			strValue = "false";
			return true;
		}
...
std::string CJsonObject::Get2(const std::string& strKey) const
{
	std::string s;
	if( !this->Get2( strKey, s ) ) return "";
	return s;
}
...
//
bool CJsonObject::Add2( const std::string& strKey, std::string strValue )
{
	this->Delete( strKey );
	return this->Add( strKey , strValue );
}

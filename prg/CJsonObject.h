...
public:     // method of ordinary json object
    ...
    bool Get2(const std::string& strKey, std::string& strValue) const;	// 数类型返回字符串形式。null空串，true/false小写。再不行才return(false) 
    std::string Get2(const std::string& strKey) const;	// 没有就返回空串	 
    ...
    bool Add2( const std::string& strKey, std::string strValue ); // 如无则新增，如有则删除再增。	
